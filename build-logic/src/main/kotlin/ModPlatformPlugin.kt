...
abstract class ModPlatformPlugin @Inject constructor() : Plugin<Project> {
	override fun apply(project: Project) = with(project) {
		...
		val inferredLoader = project.buildFile.name
			.substringAfter('.')
			.replace(".gradle.kts", "")
			.substringBefore('-')
		...
		val extension = extensions.create("platform", ModPlatformExtension::class.java).apply {
			...
			jarTask.convention(if (inferredLoaderIsFabric && !isUnobf) "remapJar" else "jar")
			sourcesJarTask.convention(if (inferredLoaderIsFabric && !isUnobf) "remapSourcesJar" else "sourcesJar")
		}
	...
	private fun Project.configurePublishing(
		...
		val enablePublishing = envTrue("ENABLE_PUBLISHING")
